...
//lines in xy plane
draw((0,0,0)--(0,2*pi,0),rgb(.1,.1,.1)+dashed+linewidth(2));
draw((2*pi,0,0)--(2*pi,2*pi,0),rgb(.1,.1,.1)+dashed+linewidth(2));

//lines on surface
triple g(real t) {return (t,0,sin(t));}
path3 mypath=graph(g,0,2*pi,operator ..);draw(mypath,rgb(.1,.1,.1)+dashed+linewidth(2));
