\begin{tikzpicture}[>=stealth]
\begin{axis}[width=\marginparwidth+25pt,%
tick label style={font=\scriptsize},axis y line=middle,axis x line=middle,name=myplot,axis on top,%
			%x=.37\marginparwidth,
			%y=.37\marginparwidth,
			%xtick={1},
			%minor x tick num=4,% 
%			extra x ticks={.33},
%			extra x tick labels={$1/3$},
			%ytick={.5},
			%minor y tick num=4,%extra y ticks={-5,-3,...,7},%
			ymin=-1.6,ymax=1.6,%
			xmin=-.7,xmax=3.1%
]

%\draw (axis cs:4,-1) node (A) {}
			%(axis cs:4,3) node (B) {} ;
%\draw [->,>=latex] (A) parabola bend (axis cs:0,1) (B);
\addplot [thick,{\coloronefill},fill={\coloronefill}, smooth,domain=75:90,samples=60] ({cos(x)*3*cos(x)},{sin(x)*cos(x)*3});

\addplot [thick,{\coloronefill},fill={\coloronefill}, smooth,domain=0:75,samples=40] ({cos(x)*(1-cos(x))},{sin(x)*(1-cos(x))});

%\addplot [thick,{\coloronefill},fill={\coloronefill}, smooth,domain=0:30,samples=20] ({cos(x)*sin(2*x)},{sin(x)*sin(2*x)}) -- (axis cs:0,0);

\addplot [thick,{\colorone}, smooth,domain=0:180,samples=60] ({cos(x)*3*cos(x)},{sin(x)*cos(x)*3});

\addplot [thick,{\colortwo}, smooth,domain=0:360,samples=60] ({cos(x)*(1-cos(x))},{sin(x)*(1-cos(x))});

\end{axis}

\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}





















