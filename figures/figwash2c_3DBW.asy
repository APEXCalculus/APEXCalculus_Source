...
draw(s2,emissive(simplesurfacepen+opacity(.4)));

triple f2(pair t) {return (cos(t.x)*(2),t.y,sin(t.x)*2);}
surface s2=surface(f2,(0,1),(2*pi,3),16,2,Spline);
draw(s2,emissive(simplesurfacepen+opacity(.2)),meshpen=apexmeshpen);

triple f2(pair t) {return (cos(t.x)*(t.y/2+1/2),t.y,sin(t.x)*(t.y/2+1/2));}
surface s2=surface(f2,(0,1),(2*pi,3),16,2,Spline);
draw(s2,emissive(simplesurfacepen+opacity(.4)),meshpen=apexmeshpen);
