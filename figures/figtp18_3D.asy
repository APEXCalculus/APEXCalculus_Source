...
//ASY file for figtp183D.asy in Chapter 12
...
//Draw surface x^2/12+y^2/6+z^2/4=1
triple f(pair t) {
  return (sqrt(12)*cos(t.x)*sin(t.y),sqrt(6)*cos(t.x)*cos(t.y),2*sin(t.x));
}
...
dotfactor=3;dot((1,2,1));

//Draw the tangent plane z=-x/3-4y/3+4
triple f(pair t) {
  return (t.x,t.y,-t.x/3-4*t.y/3+4);
}
surface s=surface(f,(0,1),(2,3),1,1,Spline);
pen p=rgb(0,0,.7);
draw(s,rgb(1,0.6,0.6)+opacity(.7),meshpen=p);
