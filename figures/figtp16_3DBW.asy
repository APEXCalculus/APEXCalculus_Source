...
//ASY file for figtp163D.asy in Chapter 12
...
real[] myzchoice={-2,2};
defaultpen(0.5mm);
pair xbounds=(-1.5,3.5);
pair ybounds=(-1.5,3.5);
...
//Draw surface z=-x^2-y^2+2
triple f(pair t) {
  return (t.x,t.y,-(t.x)^2-(t.y)^2+2);
}
...
dotfactor=3;dot((0,1,1));

//Draw the tangent plane z=-2y+3
triple f(pair t) {
  return (t.x,t.y,-2*t.y+3);
}
surface s=surface(f,(-1,0),(1,2),1,1,Spline);
pen p=rgb(0,0,.7);
draw(s,emissive(simplesurfacepen),meshpen=apexmeshpen);
