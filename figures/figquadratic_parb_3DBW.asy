...
//ASY file for figquadratic_parb.pdf in Chapter 10
...
//Draw the trace for y=0 in rgb(.3,.3,.3)
triple g(real t) {return (t,0,t^2/4);}
path3 mypath=graph(g,-4,4,operator ..); draw(mypath,rgb(.3,.3,.3));
//Draw the trace for x=0 in rgb(.3,.3,.3)
triple g(real t) {return (0,t,t^2);}
path3 mypath=graph(g,-2,2,operator ..); draw(mypath,rgb(.3,.3,.3));
//Draw the trace for z=2 in rgb(.3,.3,.3)
...
label("In plane",(-1,3,1),N);label("$z=2$",(-1,2,0.5),E);
