...
\example{ex_total_diff_10}{\textbf{Finding the total differential}\\
...
\example{ex_totaldiff1}{\textbf{Showing a function is differentiable}\\
...
more tangible way of determining whether a great number of function are differentiable or not.

\theorem{thm:diff_cont_multi}{Continuity and Differentiability of Multivariable Functions}
...
It is also possible for $f_x$ and $f_y$ to exist at a point but not be continuous at that point. In such a case, we need to use other methods to determine whether or not $f$ is differentiable at that point.
...
\example{ex_totaldiff2}{\text{Approximating with the total differential}\\
...
We let $\ddz = f(4.1,0.8) - f(4,\pi/4)$. The total differential $dy$ is approximately equal to $\ddz$, so
...
\example{ex_totaldiff3}{\textbf{Approximating an unknown function}\\
...
\example{ex_totaldiff4}{\textbf{Sensitivity analysis}\\
...
\theorem{thm:differentiable3}{Continuity and Differentiability of Functions of Three Variables}
