...
\example{ex_series1}{\textbf{Showing series diverge}
\begin{enumerate}
...
\item		The series converges if, and only if, $|r| < 1$. When $|r|<1$, $$\sum_{n=0}^\infty r^n = \frac{1}{1-r}.$$
\end{itemize}
}

According to Theorem \ref{thm:geom_series}, the series $\ds\sum_{n=0}^\infty \frac{1}{2^n} = 1+\frac12+\frac14+\cdots$ converges, and $\ds \sum_{n=0}^\infty \frac{1}{2^n} = \frac{1}{1-1/2} = 2.$ This concurs with our introductory example; while there we got a sum of 1, we skipped the first term of 1.\\
\clearpage

\example{ex_series2}{\textbf{Exploring geometric series}\\
...
This is illustrated in Figure \ref{fig:series2}(a).
...
\mtable{.55}{Scatter plots relating to the series in Example \ref{ex_series2}.}{fig:series2}{%
\begin{tabular}{c}
...
\example{ex_series6}{\textbf{Determining convergence of series}\\
...
\example{ex_series3}{\textbf{Telescoping series}\\
...
\mfigure{.7}{Scatter plots relating to the series of Example \ref{ex_series3}.}{fig:series3}{figures/figseries3}
}\\
...
\example{ex_series4}{\textbf{Evaluating series}\\
...
\example{ex_series5}{\textbf{Evaluating series}\\
