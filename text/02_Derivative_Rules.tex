...
\example{ex_deriv_rule1}{\textbf{Using Theorem \ref{thm:deriv_common} to find, and use, derivatives}\\
Let $f(x)=x^3$. 

		%\noindent\begin{minipage}[t]{.5\textwidth}
		\begin{enumerate}
		\item		Find $\fp(x)$.
		\item		Find the equation of the line tangent to $f$ at $x=-1$. 
%		\end{enumerate}
%		\end{minipage}
...
\enlargethispage{\baselineskip}
		...
		\item		To find the equation of the line tangent to $f$ at $x=-1$, we need a point and the slope. The point is $(-1,f(-1)) = (-1, -1)$. The slope is $\fp(-1)= 3$. Thus the tangent line is $y = 3(x-(-1))+(-1) = 3x+2$. 
...
{Let $f$ and $g$ be differentiable on an open interval $I$ and let $c$ be a real number. Then:
	\begin{enumerate}
	%\item		\parbox{75pt}{\textbf{Sum/Difference Rule:}}\parbox[t]{176pt}{$\ds \frac{d}{dx}\Big(f(x) \pm g(x)\Big) = \frac{d}{dx}\Big(f(x)\Big) \pm \frac{d}{dx}\Big(g(x)\Big)$ $= \rule{0pt}{15pt}\fp(x)\pm g'(x)$}
...
\example{ex_der2}{\textbf{Using the tangent line to approximate a function value}\\
...
\example{ex_high_order}{\textbf{Finding higher order derivatives}\\
