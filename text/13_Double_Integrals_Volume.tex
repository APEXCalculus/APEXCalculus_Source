...
\example{ex_double1}{\textbf{Evaluating a double integral}\\
...
\mfigure[scale=1.25,trim= 1mm 0mm 2mm 0mm,clip]{.75}{Finding the signed volume under a surface in Example \ref{ex_double1}.}{fig:double1}{figures/figdouble1}
...
%\example{ex_double2}{\textbf{Evaluating a double integral}\\
...
\example{ex_double2}{\textbf{Evaluating a double integral}\\
Evaluate $\iint_R 3xy-x^2-y^2+6\ dA$, where $R$ is the triangle bounded by $x=0$, $y=0$ and $\frac12x+y=1$, as shown in Figure \ref{fig:double2}.
...
\example{ex_double3}{\textbf{Evaluating a double integral}\\
...
\mfigure[trim=0mm 15mm 0mm 0mm,clip]{.35}{Finding the signed volume under a surface in Example \ref{ex_double3}.}{fig:double3}{figures/figdouble3}
}
{If we attempt to integrate using an iterated integral with the order $dy\ dx$, note how there are two upper bounds on $R$ meaning we'll need to use two iterated integrals. We can split the triangle into to regions along the $y$-axis, then use Theorem \ref{thm:double_prop}, part \ref{thm:double_prop_regions}.
...
\example{ex_double4}{\textbf{Evaluating a double integral}\\
...
{Graphing each curve can help us find their points of intersection; analytically, the second equation tells us that $y=x^2/4$. Substituting this value in for $y$ in the first equation gives us $x^4/16 = 4x$. Solving for $x$:
\begin{align*}
\frac{x^4}{16} &= 4x\\
x^4-64x &=0\\
x(x^3-64) &=0\\
...
%
%\example{ex_double5}{\textbf{Changing the order of integration}\\
...
%}\\
%
%\example{ex_double7}{\textbf{Changing the order of integration}\\
...
\example{ex_double6}{\textbf{Changing the order of integration}\\
...
\mfigure[scale=1.25,trim=1mm 0mm 1mm 0mm,clip]{.5}{Showing how an iterated integral used to find area also finds a certain volume.}{fig:double_summary}{figures/figdouble_summary}
