...
\example{ex_polar1}{\textbf{Plotting Polar Coordinates}\\
Plot the following polar coordinates:
$$A = P(1,\pi/4)\quad B=P(1.5,\pi)\quad C = P(2,-\pi/3)\quad D = P(-1,\pi/4)$$
}
{%
\AddToShipoutPicture*{\put(\LenToUnit{.2\paperwidth},\LenToUnit{.04\paperheight}){%
...
\keyidea{idea:polarconvert}{Converting Between Rectangular and Polar Coordinates}
...
\example{ex_polar2}{\textbf{Converting Between Polar and Rectangular Coordinates}\\
\begin{enumerate}
...
\enlargethispage{2\baselineskip}
	...
	\item \begin{itemize}
		...
		This is not the angle we desire. The range of $\tan^{-1}x $ is $(-\pi/2,\pi/2)$; that is, it returns angles that lie in the $1^\text{st}$ and $4^\text{th}$ quadrants. To find locations in the $2^\text{nd}$  and $3^\text{rd}$ quadrants, use the result of $\tan^{-1}x$ as a reference angle from the negative $x$-axis. Starting at the negative $x$-axis (i.e., with $\theta =\pi$), moving an angle of $-\pi/4$ puts the angle at $3\pi/4$. Thus the polar point is $P(\sqrt{2},3\pi/4)$.
...
\example{ex_polar3}{\textbf{Introduction to Graphing Polar Functions}\\
...
\example{ex_polar4}{\textbf{Sketching Polar Functions}\\ 
...
\example{ex_polar5}{\textbf{Sketching Polar Functions}\\
...
\example{ex_polar6}{\textbf{Converting between rectangular and polar equations.}\\
...
There are a number of basic and ``classic'' polar curves, famous for their beauty and/or applicability to other sciences. This section ends with a small gallery of some of these graphs. We encourage the reader understand how these graphs are formed, and to investigate with technology other types of polar functions.\\

\enlargethispage{2\baselineskip}
\newlength{\gallerywidth}
\setlength{\gallerywidth}{(0pt+\marginparwidth+\textwidth)/4}
\vskip \baselineskip
\noindent\hskip-\marginparwidth\hskip-25pt\rule{25pt+\marginparwidth+\textwidth}{1pt}\\

\noindent\hskip-\marginparwidth\hskip-25pt\textbf{\large Lines}\\
...
\example{ex_polar7}{\textbf{Finding points of intersection with polar curves}\\
Determine where the graphs of the polar equations $r=1+3\cos\theta$ and $r=\cos \theta$ intersect.
}
{As technology is generally readily available, it is generally a good idea to start with a graph. We have graphed the two functions in Figure \ref{fig:polar7} (a); to better discern the intersection points, part (b) of the figure zooms in around the origin.
\mtable{.7}{Graphs to help determine the points of intersection of the polar functions given in Example \ref{ex_polar7}.}{fig:polar7}{%
\begin{tabular}{c}
