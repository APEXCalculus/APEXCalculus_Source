...
The Chain Rule, as learned in Section \ref{sec:chainrule}, states that $\ds \frac{d}{dx}\Big(f\big(g(x)\big)\Big) = \fp\big(g(x)\big)g'(x)$. If $t=g(x)$, we can express the Chain Rule as 
...
Consider Figure \ref{fig:mchain_intro} in which a surface is drawn, along with a dashed curve in the $x$-$y$ plane. Restricting $f$ to just the points on this circle gives the curve shown on the surface.
...
\example{ex_mchain1}{\textbf{Using the Multivariable Chain Rule}\\
...
\example{ex_mchain2}{\textbf{Applying the Multivariable Chain Rule}\\
...
\mfigure{.5}{Plotting the path of a particle on a surface in Example \ref{ex_mchain2}.}{fig:mchain2}{figures/figmchain2}
...
\example{ex_mchain3}{\textbf{Using the Multivarible Chain Rule, Part II}\\
...
\example{ex_mchain4}{\textbf{Using the Multivarible Chain Rule, Part II}\\
...
\example{ex_mchain5}{\textbf{Implicit Differentiation}\\
