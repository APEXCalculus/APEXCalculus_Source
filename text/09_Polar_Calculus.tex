...
\example{ex_polcalc1}{\textbf{Finding $\frac{dy}{dx}$ with polar functions.}\\
Consider the lima\c con $r=1+2\sin\theta$ on $[0,2\pi]$. Find:
\begin{enumerate}
	\item The equations of the tangent and normal lines to the graph at $\theta=\pi/4$.
	\item		Where the graph has vertical and horizontal tangent lines.
\end{enumerate}
}
{\begin{enumerate}
...
\example{ex_polcalc2}{\textbf{Finding tangent lines at the pole.}\\
...
\example{ex_polcalc3}{\textbf{Area of a polar region}\\
...
\mnote{.7}{\textbf{Note:} Example \ref{ex_polcalc3} required the use of the integral $\ds\int \cos^2\theta\ d\theta$. This is handled well by using the power reducing formula as found in the back of this text. Due to the nature of the area formula, integrating $\cos^2\theta$ and $\sin^2\theta$ is required often. We offer here these indefinite integrals as a time--saving measure.\\
$$\int\cos^2\theta\ d\theta = \frac12\theta+\frac14\sin(2\theta)+C$$
$$\int\sin^2\theta\ d\theta = \frac12\theta-\frac14\sin(2\theta)+C$$
}

\example{ex_polcalc4}{\textbf{Area of a polar region}\\
Find the area of the cardiod $r=1+\cos\theta$ bound between $\theta=\pi/6$ and $\theta=\pi/3$, as shown in Figure \ref{fig:polcalc4}.
\mfigure{.45}{Finding the area of the shaded region of a cardiod in Example \ref{ex_polcalc4}.}{fig:polcalc4}{figures/figpolcalc4}
}
...
\mfigure{.8}{Illustrating area bound between two polar curves.}{fig:polarea3}{figures/figpolarea3}
...
\enlargethispage{2\baselineskip}
\example{ex_polcalc5}{\textbf{Area between polar curves}\\
Find the area bounded between the curves $r=1+\cos \theta$ and $r=3\cos\theta$, as shown in Figure \ref{fig:polcalc5}.
\mfigure{.5}{Finding the area between polar curves in Example \ref{ex_polcalc5}.}{fig:polcalc5}{figures/figpolcalc5}
}
...
\example{ex_polcalc6}{\textbf{Area defined by polar curves}\\
...
\mtable{.7}{Graphing the region bounded by the functions in Example \ref{ex_polcalc6}.}{fig:polcalc6}{%
\begin{tabular}{c}
...
The expression $x'(\theta)^2+y'(\theta)^2$ can be simplified a great deal; we leave this as an exercise and state that $$x'(\theta)^2+y'(\theta)^2 = \fp(\theta)^2+f(\theta)^2.$$ This leads us to the full arc length formula.
...
\example{ex_polcalc7}{\textbf{Arc length of a lima\c con}\\
...
L 	&= \int_0^{2\pi} \sqrt{(2\cos\theta)^2+(1+2\sin\theta)^2}\ d\theta \\
		...
		&\approx 13.3649.
\end{align*}
\mfigure{.5}{The lima\c con in Example \ref{ex_polcalc7} whose arc length is measured.}{fig:polcalc7}{figures/figpolcalc7}
...
\example{ex_polcalc8}{\textbf{Surface area determined by a polar curve}\\
...
{\mtable{.5}{Finding the surface area of a rose--curve petal that is revolved around its central axis.}{fig:polcalc8}{%
\begin{tabular}{c}
