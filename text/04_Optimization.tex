...
{\mfigure{.4}{A sketch of the enclosure in Example \ref{ex_opt2}.}{fig:opt2}{figures/figopt2a}
	...
	\item		We now reduce the fundamental equation to a single variable. In the perimeter equation, solve for $y$: $y = 50 - 1/2x$. We can now write Area as $$\text{Area} = A(x) = x(50-1/2x) = 50x - 1/2x^2.$$ Area is now defined as a function of one variable.
	\item		We want the area to be nonnegative. Since $A(x) = x(50-1/2x)$, we want $x\geq 0$ and $50-1/2x\geq 0$. The latter inequality implies that $x\leq100$, so $0\leq x\leq 100$. 
