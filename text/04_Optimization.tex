...
\example{ex_opt1}{\textbf{Optimization: perimeter and area}\\
...
%		\item		Understand the problem. Clearly identify what quantity is to be maximized or minimized. Make a sketch if helpful.
%		\item		Create functions relevant to the context of the problem, using the information given. (One of these should directly relate to the quantity to optimized. We'll call this the \textit{fundamental equation.})
%		\item		If the fundamental equation has more than one variable in it, reduce it to a single variable function using substitutions derived from the other equations.
		\item		Identify the domain of this function, keeping in mind the context of the problem.
		\item		Find the extreme values of the fundamental equation on the determined domain.
		\item		Identify the values of all relevant quantities of the problem.
...
\example{ex_opt2}{\textbf{Optimization: perimeter and area}\\
...
\ifthenelse{\boolean{longpage}}% longpage
{\noindent\hskip -20pt\begin{minipage}{\textwidth}\centering
\myincludegraphics{figures/figopt2b}
\captionsetup{type=figure}%
\caption{A sketch of the enclosure in Example \ref{ex_opt2}.}\label{fig:opt2}
\end{minipage}
} % end longpage
{\mfigure{.4}{A sketch of the enclosure in Example \ref{ex_opt2}.}{fig:opt2}{figures/figopt2a}
} %end regular page
	
	\item		We want to maximize the area; as in the example before, $$\text{Area} = xy.$$ This is our fundamental equation; since this is a function of two variables, we need another so we can reduce it one variable. 
	...
	\item		We now reduce the fundamental equation to a single variable. In the perimeter equation, solve for $y$: $y = 50 - 1/2x$. We can now write Area as $$\text{Area} = A(x) = x(50-1/2x) = 50x - 1/2x^2.$$
	\item		We want the area to be nonnegative. Since $A(x) = x(50-1/2x)$, we want $x\geq 0$ and $50-1/2x\geq 0$. The latter equation implies that $x\leq100$, so $0\leq x\leq 100$. 
	\item		We now find the extreme values. At the endpoints, the minimum is found, giving an area of 0. 
	
	We now find the critical points. We find $A'(x) = 50-x$; setting this equal to 0 and solving for $x$ returns $x=50$. This gives an area of $$A(50) = 50(25) = 1250.$$
	\item		We earlier set $y = 50-1/2x$; thus $y = 25$. Thus our rectangle will have two sides of length 25 and one side of length 50, with a total area of 1250 ft$^2$.
	\end{enumerate}
\vskip-\baselineskip
}\\

Keep in mind as we do these problems that we are practicing a \textit{process}; that is, we are learning to turn a situation into a system of equations. A key equation in that system is then optimized.\\
...
\example{ex_opt3}{\textbf{Optimization: minimizing cost}\\
...
\mfigure{.6}{Running a power line from the power station to an offshore facility with minimal cost in Example \ref{ex_opt3}.}{fig:opt3b}{figures/figopt3b}
}
...
\mfigure{.4}{Labeling unknown distances in Example \ref{ex_opt3}.}{fig:opt3c}{figures/figopt3c}
...
Evaluating the cost at $x=416.67$ gives a cost of about \$370,000. The distance the power line is laid along land is $5000-416.67 = 4583.33$ ft., and the underwater distance is $\sqrt{416.67^2+1000^2} \approx 1083$ ft.
