...
\example*{ex_der_meaning1}{\textbf{The meaning of the derivative: World Population}\\
...
\example{ex_der_meaning2}{\textbf{The meaning of the derivative: Manufacturing}\\
...
\example{ex_der_meaning3}{\textbf{Understanding the derivative: the rate of change}\\
Consider $f(x) = x^2$ as shown in Figure \ref{fig:xsquared}. It is clear that at $x=3$ the function is growing faster than at $x=1$; how much faster?
...
\example{ex_der_meaning4}{\textbf{Understanding the graph of the derivative}\\
Consider the graph of $f(x)$ (in blue)  and its derivative, $\fp(x)$ (in red) in Figure \ref{fig:fwithderiv}. %f[x_] := InterpolatingPolynomial[{-1, 0, 2}, x]*4,MAKE BIGGER, PLOT THIS WITH F' NO INTEGRATE,IDIOT
 Use these graphs to find the slopes of the tangent lines to $f$ at $x=1$, $x=2$, and $x=3$.
...
{To find the appropriate slopes of tangent lines to $f$, we need to look at the corresponding values of $\fp$.
...
\example{ex_der_meaning5}{\textbf{Approximation with the derivative}\\
...
\mfigure{.3}{Zooming in on $f$ at $x=3$ for the function given in Examples \ref{ex_der_meaning4} and \ref{ex_der_meaning5}.}{fig:fwithderivzoom3}{figures/figfwithderivzoom3}
