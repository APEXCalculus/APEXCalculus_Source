...
\example{ex_vect1}{\textbf{Using component form notation for vectors}\\
\begin{enumerate}
...
{\begin{enumerate}
	...
	$$\norm{\vec u} = \sqrt{2^2+0^2+3^2} = \sqrt{13}.$$
\end{enumerate}
\vskip-\baselineskip
}\\
...
\example{ex_vect2}{\textbf{Adding vectors}\\
...
\example{ex_vect4}{\textbf{Vector Subtraction}\\
...
\example{ex_vect3}{\textbf{Scaling vectors}\\
\begin{enumerate}
...
\item	We compute $2\vec v$:
	...
	These are sketched in Figure \ref{fig:vect3}. Make note that $2\vec v$ does not start at the terminal point of $\vec v$; rather, its initial point is also the origin. 
...
\snorm{\frac{1}{\norm{\vec v}}\vec v} &= \frac{1}{\norm{\vec v}}\norm{\vec v} & \text{\small (we can pull out $\ds \frac{1}{\norm{\vec v}}$ as it is a scalar)}\\
								&= 1.
\end{align*}			
So the vector of length 10 in the direction of $\vec v$ is $\ds 10\cdot\frac{1}{\norm{\vec v}}\cdot\vec v.$ An example will make this more clear.\\

\example{ex_vect5}{\textbf{Using Unit Vectors}\\
...
{\begin{itemize}
	...
	\item	Vectors $\vec v_1$ and $\vec v_2$ are \textbf{parallel} if their respective unit vectors are parallel.
\end{itemize}
}

It is equivalent to say that vectors $\vec v_1$ and $\vec v_2$ are parallel if there is a scalar $c\neq 0$ such that $\vec v_1 = c\vec v_2$.
...
\example{ex_vect6}{\textbf{Finding Component Forces}\\
...
\example{ex_vect7}{\textbf{Using standard unit vectors}\\
\begin{enumerate}
...
\example{ex_vect8}{\textbf{Finding Component Force}\\
A weight of 25 is suspended from a chain of length 2ft while a wind pushes the weight to the right with constant force of 5lb as shown in Figure \ref{fig:vect8}. What angle will the chain make with the vertical as a result of the wind's pushing? How much higher will the weight be?
...
This is enough to determine $\vec F_c$ already! We know that $F_c = \la -5,25\ra.$ We can use this to find the magnitude $m$:
