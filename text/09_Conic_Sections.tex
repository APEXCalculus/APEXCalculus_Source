...
The three ``most interesting'' conic sections are given in the top row of Figure \ref{fig:nondeg_conic}. They are the parabola, the ellipse (which includes circles) and the hyperbola. In each of these cases, the plane does not intersect the tips of the cones (usually taken to be the origin). \\
...
\caption{Nondegenerate Conic Sections}\label{fig:nondeg_conic}
\end{minipage}
\vskip\baselineskip



When the plane does contain the origin, three \textbf{degenerate} cones can be formed as shown the bottom row of  Figure \ref{fig:nondeg_conic}: a point, a line, and crossed lines. We focus here on the nondegenerate cases. 
...
These two distances are equal. Setting $d_1=d_2$, we can solve for $y$ in terms of $x$:
	...
	\example{ex_conic1}{\textbf{Finding the equation of a parabola}\\
	Give the equation of the parabola with focus at $(1,2)$ and directrix at $y=3$.}
	{The vertex is located halfway between the focus and directrix, so $(h,k) = (1,4)$. This gives $p=-0.5$. Using Key Idea \ref{idea:parabola} we have the equation of the parabola as $$y=\frac{1}{4(-0.5)}(x-1)^2+4 = -\frac12(x-1)^2+2.5.$$
	\mfigure{.35}{The parabola described in Example \ref{ex_conic1}.}{fig:conic1}{figures/figconic1}
	The parabola is sketched in Figure \ref{fig:conic1}.
	}\\
	
	\example{ex_conic2}{\textbf{Finding the focus and directrix of a parabola}\\
...
\mfigure{.7}{The parabola described in Example \ref{ex_conic2}. The distances from a point on the parabola to the focus and directrix is given.}{fig:conic2}{figures/figconic2}
	...
	One of the fascinating things about the nondegenerate conic sections is their reflective properties. For a parabola, the reflective property is this:
	...
	\end{minipage}
	\mfigure{.35}{Illustrating the parabola's reflective property.}{fig:conic_reflect}{figures/figconic_reflect}
...
\mfigure{.75}{Labeling the significant features of an ellipse.}{fig:ellipse_label}{figures/figellipse_label}
	...
	\example{ex_conic3}{\textbf{Finding the equation of an ellipse}\\
...
\example{ex_conic4}{\textbf{Graphing an ellipse}\\
Graph the ellipse defined by $4x^2+9y^2-8x-36y=-4$.}
{It is simple to graph an ellipse once it is in general form. In order to put the given equation in general form, we must complete the square with both the $x$ and $y$ terms. We first rewrite the equation by regrouping:
...
When $a=b$, we have a circle. The general equation becomes $$\frac{(x-h)^2}{a^2} + \frac{(y-k)^2}{a^2} = 1 \quad \Rightarrow (x-h)^2 + (y-k)^2 = a^2,$$ the familiar equation of the circle centered at $(h,k)$ with radius $a$.  The circle has ``two'' foci, but they lie on the same point, the center of the circle. 
...
\example{ex_conic5}{\textbf{Graphing a hyperbola}\\
...
\mfigure{.7}{Graphing the hyperbola in Example \ref{ex_conic5}.}{fig:conic5}{figures/figconic5}
...
\example{ex_conic6}{\textbf{Graphing a hyperbola}\\
...
The ``difference of distances'' is 340; this is also the distance between vertices of the hyperbola. So we know $2a= 340$. Positions $A$ and $B$ lie on the foci, so $2c=1000$. From this we can find $b\approx 470$ and can sketch the hyperbola, given in part (b) of the figure. We only care about the side closest to $A$. 
...
\vskip 10pt
\noindent\hskip-150pt\begin{minipage}{\textwidth+150pt}
\centering
\begin{tabular}{ccccc}
