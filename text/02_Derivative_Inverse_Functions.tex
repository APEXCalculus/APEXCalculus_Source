...
\mfigure{.6}{A function $f$ along with its inverse $f^{-1}$. (Note how it does not matter which function we refer to as $f$; the other is $f^{-1}$.)}{fig:inverse1}{figures/figinverse1}
...
\mfigure{.35}{Corresponding tangent lines drawn to $f$ and $f^{-1}$.}{fig:inverse2}{figures/figinverse2}
...
\example{ex_deriv_arcsin}{\textbf{Finding the derivative of an inverse trigonometric function}\\
Let $y = \arcsin x = \sin^{-1} x$. Find $y'$ using Theorem \ref{thm:deriv_inverse_functions}.}
...
Therefore $\cos (\sin^{-1} x) = \cos y = \sqrt{1-x^2}/1 = \sqrt{1-x^2}$, resulting in $$\frac{d}{dx}\big(\arcsin x\big) = g'(x) = \frac{1}{\sqrt{1-x^2}}.$$
\vskip-\baselineskip
}\\
...
{\mfigure{.5}{Graphs of $\sin x$ and $\sin^{-1}x$ along with corresponding tangent lines.}{fig:inverse4}{figures/figinverse4a}}
...
{The inverse trigonometric functions are differentiable on their domains (as listed in Figure \ref{fig:domain_trig}) and their derivatives are as follows:\\
...
\example{ex_deriv_lnx}{\textbf{Finding the derivative of $y=\ln x$}\\
