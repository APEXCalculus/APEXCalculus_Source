...
\example{ex_basic_limit_1}{\textbf{Using basic limit properties}\\
...
\example{ex_limit_rat}{\textbf{Finding a limit of a rational function}\\
...
\example{ex_limit_1}{\textbf{Evaluating limits analytically}\\
...
\item		Applying Theorem \ref{thm:lim_continuous} directly gives $$\ds \lim_{x\to \pi/2} \cos x\sin x = \cos (\pi/2)\sin(\pi/2) = 0\cdot 1 = 0.$$
...
We can also use the Composition limit rule. Using Theorem \ref{thm:lim_continuous}, we have $\ds \lim_{x\to 1}\ln x = \ln 1 = 0$. Thus $$\ds \lim_{x\to 1} e^{\ln x} = \lim_{x\to 0} e^x = e^0 = 1.$$ Both approaches are valid, giving the same result.
...
\example{ex_limit_sinx_prove}{\textbf{Using the Squeeze Theorem}\\
...
\example{ex_limit_onept}{\textbf{Using algebra to evaluate a limit}\\
...
{We begin by attempting to apply Theorem \ref{thm:lim_continuous} and substituting 1 for $x$ in the quotient. This gives:
		$$\lim_{x\to 1}\frac{x^2-1}{x-1} = \frac{1^1-1}{1-1} = \raisebox{8pt}{\text{``\ }}\frac{0}{0}\raisebox{8pt}{\text{\ ''}},$$ and indeterminate form. We cannot apply the theorem.
...
\example{ex_limit_allbut1}{\textbf{Evaluating a limit using Theorem \ref{thm:limit_allbut1}}\\
...
\example{ex_limit_diffquot}{\textbf{Evaluating the limit of a difference quotient}\\
Let $f(x) = -1.5x^2+11.5x$; find $\ds \lim_{h\to 0}\frac{f(1+h)-f(1)}{h}.$}
{Since $f$ is a polynomial, our first attempt should be to employ Theorem \ref{thm:lim_continuous} and substitute 0 for $h$. However, we see that this gives us \zerooverzero. Knowing that we have a rational function hints that some algebra will help. Consider the following steps:
		\begin{align*}
