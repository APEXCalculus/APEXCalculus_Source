...
\example{ex_parcalc1}{\textbf{Tangent and Normal Lines to Curves}\\
...
\end{enumerate}
	...
	{\begin{enumerate}
		...
		\begin{itemize}
			\item The equation of the tangent line is $\ds y=\frac12(x-31)+26$.
	...
	\example{ex_parcalc2}{\textbf{Tangent and Normal Lines to a Circle}\\
	\begin{enumerate}
...
\mfigure{.8}{Illustrating how a circle's normal lines pass through its center.}{fig:parcalc2}{figures/figparcalc2}
		\end{enumerate}
...
\example{ex_parcalc3}{\textbf{Tangent Lines When $\frac{dy}{dx}$ is Not Defined}\\
...
\mfigure{.5}{A graph of an astroid.}{fig:parcalc3}{figures/figplanecurve1}
...
\example{ex_parcalc4}{\textbf{Concavity of Plane Curves}\\
...
\example{ex_parcalc5}{\textbf{Concavity of Plane Curves}\\
...
{Let $x=f(t)$ and $y=g(t)$ be parametric equations with $\fp$ and $g'$ continuous on some open interval $I$ containing $t_1$ and $t_2$ on which the graph traces itself only once. The arc length of the graph, from $t=t_1$ to $t=t_2$, is
...
\example{ex_parcalc6}{\textbf{Arc Length of a Circle}\\
...
\example{ex_parcalc7}{\textbf{Arc Length of a Parametric Curve}\\
...
\example{ex_parcalc8}{\textbf{Surface Area of a Solid of Revolution}\\
...
\mfigure[scale=1.5,trim=5mm 5mm 5mm 5mm,clip]{.45}{Rotating a teardrop shape about the $x$-axis in Example \ref{ex_parcalc8}.}{fig:parcalc8}{figures/figparcalc8}
}\\
