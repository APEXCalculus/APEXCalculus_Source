...
\example{ex_defint3}{\textbf{Finding position using velocity}\\
...
\noindent\parbox{\specialboxlength}{\centering (area  under $f$ and above $x$--axis on $[a,b]$) -- (area above $f$ and under $x$--axis on $[a,b]$).}\\
...
\example{ex_defint4}{\textbf{Evaluating definite integrals}\\
...
{\begin{enumerate}
		\item		$ \int_0^3 f(x)\ dx$ is the area under $f$ on the interval $[0,3]$. This region is a triangle, so the area is $\frac12(3)(1) = 1.5$. 
...
\mfigure{.8}{A graph of $5f$ in Example \ref{ex_defint4}. (Yes, it looks just like the graph of $f$ in Figure \ref{fig:defint4}, but you wouldn't have believed that unless we also showed you.)}{fig:defint4a}{figures/figdefint4a}		
...
\clearpage
\example{ex_defint5}{\textbf{Evaluating definite integrals using Theorem \ref{thm:defintprop}.}\\
Consider the graph of a function $f(x)$ shown in Figure \ref{fig:defint5}. 

\mfigure{.8}{A graph of a function in Example \ref{ex_defint5}.}{fig:defint5}{figures/figdefint5}
...
The area definition of the definite integral allows us to compute the definite integral of some simple functions.\\

\example{ex_defint8}{\textbf{Evaluating definite integrals using geometry}\\
Evaluate the following definite integrals:
		$$1. \ \int_{-2}^5 (2x-4)\ dx \qquad 2.\ \int_{-3}^3 \sqrt{9-x^2}\ dx.$$

}
{\mfigure{.35}{A graph of $f(x) = 2x-4$ in Example \ref{ex_defint8}.}{fig:defint8a}{figures/figdefint8a}

\begin{enumerate}
		\item		It is useful to sketch the function in the integrand, as shown in Figure \ref{fig:defint8a}. We see we need to compute the areas of two regions, which we have labeled $R_1$ and $R_2$. Both are triangles, so the area computation is straightforward:
			$$R_1: \frac12(4)(8) = 16 \qquad R_2: \frac12(3)6 = 9.$$ 
Region $R_1$ lies under the $x$--axis, hence it is counted as negative area (we can think of the height as being ``$-8$''), so $$\int_{-2}^5(2x-4)\ dx = 9-16 = -7.$$
		\item		Recognize that the integrand of this definite integral is a half circle, as sketched in Figure \ref{fig:defint8b}, with radius 3. Thus the area is:
		$$\int_{-3}^3 \sqrt{9-x^2}\ dx = \frac12\pi r^2 = \frac 92\pi.$$
\vskip-\baselineskip
}\\

\mfigure{.8}{A graph of $f(x) = \sqrt{9-x^2}$ in Example \ref{ex_defint8}.}{fig:defint8b}{figures/figdefint8b}
\end{enumerate}

\example{ex_defint6}{\textbf{Understanding motion given velocity}\\
...
\mfigure{.55}{A graph of a velocity in Example \ref{ex_defint6}.}{fig:defint6}{figures/figdefint6}
}
...
\mfigure{.3}{What is the area below $y=x^2$ on $[0,3]$? The region is not a usual geometric shape.}{fig:defint7}{figures/figdefint7}
