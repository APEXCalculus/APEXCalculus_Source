...
\example{ex_extval1}{\textbf{Approximating extreme values}\\
Consider $f(x) = 2x^3-9x^2$ on $I=[-1,5]$, as graphed in Figure \ref{fig:extval1}. Approximate the extreme values of $f$.

\mfigure{.6}{A graph of $f(x) = 2x^3-9x^2$ as in Example \ref{ex_extval1}.}{fig:extval1}{figures/figextval1}}
...
\mnote{.45}{\textbf{Note:} The terms \textit{local minimum} and \textit{local maximum} are often used as synonyms for \textit{relative minimum} and \textit{relative maximum}.}
...
\example{ex_extval2}{\textbf{Approximating relative extrema}\\
...
\mfigure{.8}{A graph of $f(x) = (3x^4-4x^3-12x^2+5)/5$ as in Example \ref{ex_extval2}.}%
...
\example{ex_extval3}{\textbf{Approximating relative extrema}\\
Approximate the relative extrema of $f(x) = (x-1)^{2/3}+2$, shown in Figure \ref{fig:extval3}. At each of these points, evaluate $\fp$.

\mfigure{.55}{A graph of $f(x) = (x-1)^{2/3}+2$ as in Example \ref{ex_extval3}.}{fig:extval3}{figures/figextval3}
}
...
\example{ex_extval4}{\textbf{Finding extreme values}\\
...
\example{ex_extval5}{\textbf{Finding extreme values}\\
...
\clearpage
\example{ex_extval6}{\textbf{Finding extreme values}\\
...
\example{ex_extval7}{\textbf{Finding extreme values}\\
...
%}{}
%
%From the table we see that the maximum of $f$ is 1 and the minimum is 0. A graph of $f$, the upper half of the unit circle, is given in Figure \ref{fig:extval7}.\\
%\ifthenelse{\boolean{longpage}}{%
%\mfigure{.35}{A graph of $f(x)=\sqrt{1-x^2}$ on $[-1,1]$ as in Example \ref{ex_extval7}.}{fig:extval7}{figures/figextval7}
%}{}
%\begin{minipage}{.37\textwidth}
%\centering
%\begin{tabular}{cc} $x$ & $f(x)$ \\ \hline \rule{0pt}{10pt} $-1$ & 0 \\ 0 & 1 \\ 1 & 0 \end{tabular}
%\captionsetup{type=figure}%
%\caption{Finding the extrema of the half--circle in Example \ref{ex_extval7}.}
%\label{table:ext7}
%\end{minipage}\hskip .1\textwidth
%\begin{minipage}{.48\textwidth}
%\centering
%\myincludegraphics{figures/figextval7}
%\captionsetup{type=figure}
%\caption{A graph of $f(x)=\sqrt{1-x^2}$ on $[-1,1]$ as in Example \ref{ex_extval7}.}
%\label{fig:extval7}
%\end{minipage}
}\\
