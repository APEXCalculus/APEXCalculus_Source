...
\example{ex_pareq1}{\textbf{Plotting parametric functions} \\
Plot the graph of the parametric equations $x=t^2$, $y=t+1$ for $t$ in $[-2,2]$.}
{We plot the graphs of parametric equations in much the same manner as we plotted graphs of functions like $y=f(x)$: we make a table of values, plot points, then connect these points with a ``reasonable'' looking curve. Figure \ref{fig:pareq1table} shows such a table of values; note how we have 3 columns.

The points $(x,y)$ from the table are plotted in Figure \ref{fig:pareq1}. The points have been connected with a smooth curve. Each point has been labeled with its corresponding $t$-value. These values, along with the two arrows along the curve, are used to indicate the \textbf{orientation} of the graph. This information helps us determine the direction in which the graph is ``moving.''
...
\example{ex_pareq2}{\textbf{Plotting parametric functions} \\ Sketch the graph of the parametric equations $x=\cos^2t$, $y=\cos t+1$ for $t$ in $[0,\pi]$.}
{We again start by making a table of values in Figure \ref{fig:pareq2_table}, then plot the points $(x,y)$ on the Cartesian plane in Figure \ref{fig:pareq2}.
...
\example{ex_pareq3}{\textbf{Shifting the graph of parametric functions}\\ Sketch the graph of the parametric equations $x=t^2+t$, $y=t^2-t$.  Find new parametric equations that shift this graph to the right 3 places and down 2.}
...
\example{ex_pareq4}{\textbf{Graphs that cross themselves}\\ Plot the parametric functions $x=t^3-5t^2+3t+11$ and $y=t^2-2t+3$ and determine the $t$-values where the graph crosses itself.}
...
\example{ex_pareq5}{\textbf{Converting from rectangular to parametric}\\
...
{We start by computing $\frac{dy}{dx}$: $y' = 2x$. Thus we set $t=2x$. We can solve for $x$ and find $x= t/2$. Knowing that $y=x^2$, we have $y= t^2/4$. Thus parametric equations for the parabola $y=x^2$ are $$x=t/2 \quad y=t^2/4.$$
...
\example{ex_pareq6}{\textbf{Converting from rectangular to parametric}\\
...
\example{ex_pareq8}{\textbf{Eliminating the parameter}\\
...
The Pythagorean Theorem can also be used to identify parametric equations for hyperbolas. We give the parametric equations for ellipses and hyperbolas in the following Key Ideas.
...
\example{ex_pareq9}{\textbf{Determine where a curve is not smooth}\\
