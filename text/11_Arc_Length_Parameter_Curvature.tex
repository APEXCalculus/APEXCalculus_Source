...
\mtable{.7}{Introducing the arc length parameter.}{fig:vvfarc_intro1}{%
\begin{tabular}{c}
...
\example{ex_vvfarc1}{\textbf{Finding the arc length parameter}\\
...
\example{ex_curvature2}{\textbf{Finding the curvature of a circle}\\
...
Let a point $P$ on a smooth curve $C$  be given, and let $\kappa$ be the curvature of the curve at $P$. A circle that:
	...
	\end{itemize}
is the \textbf{osculating circle}, or \textbf{circle of curvature}, to $C$ at $P$, and $r$ is the \textbf{radius of curvature}. Figure \ref{fig:curvature_osculate} shows the graph of the curve seen earlier in Figure \ref{fig:curvatureintro} and its osculating circles at $A$ and $B$. A sharp turn corresponds to a circle with a small radius; a gradual turn corresponds to a circle with a large radius. Being able to think of curvature in terms of the radius of a circle is very useful.
...
\example{ex_curvature3}{\textbf{Finding curvature}\\
...
\example{ex_curvature4}{\textbf{Finding curvature}\\
...
Let $\vec r(t)$ be a position function of an object, with velocity $\vvt = \vrp(t)$ and acceleration $\vec a(t)=\vrp'(t)$. In Section \ref{sec:tan_norm} we established that acceleration is in the plane formed by $\vec T(t)$ and $\vec N(t)$, and that we can find scalars $a_\text{T}$ and $a_\text{N}$ such that $$\vat = a_\text{T}\vec T(t) + a_\text{N}\vec N(t).$$ Theorem \ref{thm:acc_plane} gives formulas for $a_\text{T}$ and $a_\text{N}$:
...
\example{ex_curvature5}{\textbf{Curvature and road design}\\
