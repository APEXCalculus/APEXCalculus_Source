...
\example{ex_ibp1}{\textbf{Integrating using Integration by Parts}\\
...
$$\int x\cos x = x\sin x - \int \sin x \,dx.$$
...
\example{ex_ibp2}{\textbf{Integrating using Integration by Parts}\\
...
\example{ex_ibp3}{\textbf{Integrating using Integration by Parts}\\
...
\example{ex_ibp4}{\textbf{Integrating using Integration by Parts}\\
...
The integral on the right is not much different than the one we started with, so it seems like we have gotten nowhere. Let's stick keep working and apply Integration by Parts to the new integral, using $u=e^x$ and $dv = \sin x\,dx$. This leads us to the following:\\ %Then we get $du=e^x\,dx$ and $v=-\cos x$.  
...
\end{align*}
It seems we are back right where we started, as the right hand side contains $\int e^x\cos x\,dx$.  But this actually a good thing.  
...
\example{ex_ibp6}{\parbox[t]{220pt}{\textbf{Integrating using Integration by Parts: antiderivatives of $\arctan x$}}\\
...
The integral on the right can be done by substitution.  Taking $u=1+x^2$, we get $du=2x\,dx$.  The integral then becomes
...
\example{ex_ibp8}{\textbf{Integration by Parts after substitution}\\
...
\example{ex_ibp7}{\textbf{Definite integration using Integration by Parts}\\
