...
\example{ex_taypoly1}{\textbf{Finding and using Maclaurin polynomials}
\begin{enumerate}
...
\example{ex_taypoly2}{\textbf{Finding and using Taylor polynomials}
\begin{enumerate}
...
\mtable{.45}{Derivatives of $\ln x$ evaluated at $x=1$.}{fig:taypoly2a}{%
\begin{tabular}{lll}
...
\mfigure{.55}{A plot of $y=\ln x$ and its 20$^\text{th}$ degree Taylor polynomial at $x=1$.}{fig:taypoly2c}{figures/figtaypoly2c}
\end{enumerate}
\vskip-1.5\baselineskip
}\\
...
Taylor polynomials are used to approximate functions $f(x)$ in mainly two situations:
	...
	\mnote{.35}{\textbf{Note:} Even though Taylor polynomials \emph{could} be used in calculators and computers to calculate values of trigonometric functions, in practice they generally aren't. Other more efficient and accurate methods have been developed, such as the CORDIC algorithm.}
...
\example{ex_taypoly3}{\textbf{Finding error bounds of a Taylor polynomial}\\
...
In reality, our approximation was only off by about 0.07. However, we are approximating ostensibly since we do not know the real answer. In order to be assured that we have a good approximation, we would have to resort to using a polynomial of higher degree.
\end{enumerate}
\vskip-\baselineskip
}\\
...
\example{ex_taypoly4}{\textbf{Finding sufficiently accurate Taylor polynomials}\\
...
\example{ex_taypoly5}{\textbf{Finding and using Taylor polynomials}
\begin{enumerate}
...
\example{ex_taypoly6}{\textbf{Approximating an unknown function}\\
...
It is beyond the scope of this text to pursue error analysis when using Taylor polynomials to approximate solutions to differential equations. This topic is often broached in introductory Differential Equations courses and usually covered in depth in Numerical Analysis courses.
