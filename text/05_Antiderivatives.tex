...
\example{ex_anti2}{\textbf{Evaluating indefinite integrals}\\
Evaluate $\displaystyle \int \sin x\ dx.$}
{We are asked to find a function $F(x)$ such that $\Fp(x) = \sin x$. Some thought will lead us to one solution: $F(x) = -\cos x$, because $\frac{d}{dx}(-\cos x) = \sin x$.
...
We can view integration in the following way. Let $dy = \sin x\ dx$ and rewrite 
...
\example{ex_anti3}{\textbf{Evaluating indefinite integrals}\\
...
\begin{itemize}
	\item		Rule \#1 states $\int c\cdot f(x)\ dx = c\cdot \int f(x)\ dx$. We can temporarily ignore constants when finding antiderivatives, just as we did when computing derivatives (i.e., $\frac{d}{dx}\big(3x^2\big)$ is just as easy to compute as $\frac{d}{dx}\big(x^2\big)$). An example:
	...
	\item		Rule \#2 states we can split integrals apart when the integrand contains terms that are added/subtracted, as we did in Example \ref{ex_anti3}. So:
	\begin{align*}
...
\example{ex_anti4}{\textbf{Solving initial value problems}\\
...
\example{ex_anti5}{\textbf{Solving initial value problems}\\
