...
\example{ex_dotp1}{\textbf{Evaluating dot products}\\
\begin{enumerate}
...
The same is actually true of 2 vectors in space: given $\vec u$ and $\vec v$ in $\mathbb{R}^3$ with the same initial point, there is a plane that contains both $\vec u$ and $\vec v$. (When $\vec u$ and $\vec v$ are co-linear, there are infinite planes that contain both vectors.) In that plane, we can again find an angle $\theta$ between them (and again, $0\leq \theta\leq \pi$). This is illustrated in Figure \ref{fig:dotpangle} (b).
...
\example{ex_dotp2}{\textbf{Using the dot product to find angles}\\
...
\example{ex_dotp3}{\textbf{Using the dot product to find angles}\\
...
\example{ex_dotp8}{\textbf{Finding orthogonal vectors}\\
...
\end{equation}
\mtable{.50}{Developing the construction of the \emph{orthogonal projection}.}{fig:dotpproj}{%
\begin{tabular}{c}
...
\example{ex_dotp4}{\textbf{Computing the orthogonal projection}\\
\begin{enumerate}
...
\mtable{.6}{Graphing the vectors used in Example \ref{ex_dotp4}.}{fig:dotp4}{%
\begin{tabular}{c}
...
\example{ex_dotp5}{\textbf{Orthogonal decomposition of vectors}\\
\begin{enumerate}
...
\example{ex_dotp6}{\textbf{Orthogonally decomposing a force vector}\\
...
\example{ex_dotp7}{\textbf{Computing work}\\
