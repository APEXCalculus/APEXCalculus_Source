...
Let's see how it works with the equation above.\\

\example{ex_implicit1}{\textbf{Using Implicit Differentiation}\\
Find $y'$ given that $\sin(y) + y^3=6-x^3$.}
...
\example{ex_implicit2}{\textbf{Using Implicit Differentiation to find a tangent line}\\
...
{In Example \ref{ex_implicit1} we found that $$y' = \frac{-3x^2}{\cos y +3y^2}.$$ We find the slope of the tangent line at the point  $(\sqrt[3]6,0)$ by substituting $\sqrt[3]6$ for $x$ and $0$ for $y$. Thus at the point $(\sqrt[3]6,0)$, we have the slope as $$y' = \frac{-3(\sqrt[3]{6})^2}{\cos 0 + 3\cdot0^2} = \frac{-3\sqrt[3]{36}}{1} \approx -9.91.$$
...
\example{ex_implicit3}{\textbf{Using Implicit Differentiation}\\
Given the implicitly defined function $y^3+x^2y^4=1+2x$, find $y'$.}
{We will take the implicit derivatives term by term. The derivative of $y^3$ is $3y^2y'$.  
...
\example{ex_implicit5}{\textbf{Using Implicit Differentiation}\\
...
\example{ex_implicit7}{\textbf{Finding slopes of tangent lines to a circle}\\
...
\example{ex_implicit8}{\textbf{Using the Power Rule}\\
...
\example{ex_implicit9}{\textbf{Finding the second derivative}\\
Given $x^2+y^2=1$, find $\frac{d^2y}{dx^2} = y''$. }
{We found that $y' = \frac{dy}{dx} = -x/y$ in Example \ref{ex_implicit7}. To find $y''$, we apply implicit differentiation to $y'$.
\begin{align*}
...
\intertext{replace $y'$ with $-x/y$:}
		...
		&= -\frac{y+x^2/y}{y^2}
\end{align*}
While this is not a particularly simple expression, it is usable. We can see that $y''>0$ when $y<0$ and $y''<0$ when $y>0$. In Section \ref{sec:concavity}, we will see how this relates to the shape of the graph.
}\\
...
\mfigure{.6}{A plot of $y=x^x$.}{fig:logdiffa}{figures/figlogdiffa}
...
\example{ex_implicit10}{\textbf{Using Logarithmic Differentiation}\\
...
\frac{y'}{y} &= \ln x + 1\\
y' &= \parbox{50pt}{$y\big(\ln x+1\big)$} \text{\small (substitute $y=x^x$)}\\
...
%
%Implicit differentiation can be used in some surprising ways.  For instance, it can be used to find the derivative of $\arctan x$.  The key is if $y=\arctan x$, then $\tan y = x$.  This equation is now written implicitly and differentiate to get $\sec^2(y)y' = 1$.  Therefore, $y' = 1/\sec^2(y)$.  Recall that $\sec y = 1/\cos y$, so we have $y'=\cos^2{y}$.
%
...
%
%This idea can be expanded to take the derivative of any inverse function.  Given $y=f^{-1}(x)$, we can write $f(y)=x$.  Taking the derivative implicitly, we get $f'(y)y' = 1$.  Solving for $y'$ and plugging in $y=f^{-1}(x)$, we get the following:
%$$(f^{-1}(x))' = \frac1{f'(f^{-1}(x))},$$
