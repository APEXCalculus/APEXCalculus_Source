...
Consider the interval from $t=2$ to $t=3$ (just before the riders hit the ground). On that interval, the average velocity is 
		$$\frac{f(3)-f(2)}{3-2} = \frac{f(3)-f(3)}{1} =-80\ \text{ft/s},$$
...
\definition{def:derivative_at_a_point}{Derivative at a Point}{Let $f$ be a continuous function on an open interval $I$ and let $c$ be in $I$. The \textbf{derivative of $f$ at $c$}, denoted $\fp(c)$, is $$\lim_{h\to 0}\frac{f(c+h)-f(c)}{h},$$ provided the limit exists. If the limit exists, we say that \textbf{$f$ is differentiable at $c$}; if the limit does not exist, then \textbf{$f$ is not differentiable at $c$}. If $f$ is differentiable at every point in $I$, then \textbf{$f$ is differentiable on $I$}.
}

\definition{def:tangent_line}{Tangent Line}{Let $f$ be continuous on an open interval $I$ and differentiable at $c$, for some $c$ in $I$. The line $\ell(x) = \fp(c)(x-c)+f(c)$ is the \textbf{tangent line} to $f$ at $c$; that is, it is the line through $(c,f(c))$ whose slope is the derivative of $f$ at $c$.}



Some examples will help us understand these definitions.\\

\example{ex_derv_point1}{\textbf{Finding derivatives and tangent lines}\\
Let $f(x) = 3x^2+5x-7$. Find: 

\noindent\begin{minipage}[t]{.5\textwidth}
	\begin{enumerate}
	\item		$\fp(1)$
	\item		The tangent line to $f$ at $x=1$.
	\end{enumerate}
	\end{minipage}
	\noindent\begin{minipage}[t]{.5\textwidth}
	\begin{enumerate}\addtocounter{enumi}{2}
	\item		$\fp(3)$
	\item		The tangent line to $f$ at $x=3$.
	\end{enumerate}
	\end{minipage}
}
{	\begin{enumerate}
	...
	\item		The tangent line at $x=3$ has slope $23$ and goes through the point $(3,f(3)) = (3,35)$. Thus the tangent line is $y=23(x-3)+35 = 23x-34$.
		\end{enumerate}

\mfigure{.7}{A graph of $f(x) = 3x^2+5x-7$ and its tangent lines at $x=1$ and $x=3$.}{fig:tangent1}{figures/figtangent1}

A graph of $f$ is given in Figure \ref{fig:tangent1} along with its tangent lines at $x=1$ and $x=3$.
}\\
...
{Let $f$ be continuous on an open interval $I$ and differentiable at $c$, for some $c$ in $I$. The \textbf{normal line} to $f$ at $c$ is the line 
...
\example{ex_normal1}{\textbf{Finding equations of normal lines}\\
Let $f(x) = 3x^2+5x-7$, as in Example \ref{ex_derv_point1}. Find the equations of the normal lines to $f$ at $x=1$ and $x=3$.
}
...
We also found that $\fp(3) = 23$, so the normal line to $f$ at $x=3$ will have slope $-1/23$. An equation for the normal line is $$n(x) = \frac{-1}{23}(x-3)+35.$$ 
\vskip-\baselineskip
}\\
...
\example{ex_der_line}{\textbf{Finding the Derivative of a Line}\quad Consider $f(x) = 3x+5$. Find the equation of the tangent line to $f$ at $x=1$ and $x=7$.}
...
It is now easy to see that the tangent line to $f$ at $x=1$ is just $f$, with the same being true for $x=7$.
}\\

We often desire to find the tangent line to a function without knowing the actual derivative of the function. In these cases, the best we may be able to do is approximate the tangent line. We demonstrate this in the next example.\\
...
\example{ex_der_num_approx}{\textbf{Numerical Approximation of the Tangent Line}\quad Give an approximation to the tangent line of $f(x)=\sin x$ at $x=0$.}
...
Thus our approximation to the tangent line is $y = 0.9983(x-0) +0 = 0.9983x$; it is graphed in Figure \ref{fig:tangentsinx}. The graph seems to imply the approximation is rather good.
}\\
...
{Let $f$ be a differentiable function on an open interval $I$. The function $$\fp(x) = \lim_{h\to 0} \frac{f(x+h)-f(x)}{h}$$ is \textbf{the derivative of $f$}.\\
...
\example{ex_deriv1}{\textbf{Finding the derivative of a function}\\
...
\example{ex_deriv2}{\textbf{Finding the derivative of a function}\\
...
\example{ex_deriv_sinx}{\textbf{Finding the derivative of a function}\\
...
\example{ex_not_diff}{\textbf{Finding the derivative of a piecewise defined function}\\
...
\example{ex_diff_piecewise}{\textbf{Finding the derivative of a piecewise defined function}\\
