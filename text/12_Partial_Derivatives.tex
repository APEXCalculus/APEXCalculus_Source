...
\mtable{.65}{By fixing $y=2$, the surface $f(x,y) = x^2+2y^2$ is a curve in space.}{fig:partialintro}
{\begin{tabular}{c}
...
The key notion to extract from this example is: by treating $y$ as a constant (it does not vary) we can consider how $z$ changes with respect to $x$. In a similar fashion, we can hold $x$ constant and consider how $z$ changes with respect to $y$. This is the underlying principle of \textbf{partial derivatives}. We state the formal, limit--based definition first, then show how to compute these partial derivatives without directly taking limits.

%\enlargethispage{4\baselineskip}
\definition{def:partial_derivative}{Partial Derivative, Differentiable}
...
\example{ex_partial1}{\textbf{Computing partial derivatives with the limit definition}\\
...
\example{ex_partial2}{\textbf{Finding partial derivatives}\\
...
We have shown \textit{how} to compute a partial derivative, but it may still not be clear what a partial derivative \textit{means}. Given $z=f(x,y)$, $f_x(x,y)$ measures the rate at which $z$ changes as only $x$ varies: $y$ is held constant. 
...
\example{ex_partial3}{\textbf{Evaluating partial derivatives}\\
...
\example{ex_partial6}{\textbf{Second partial derivatives}\\
...
\example{ex_partial7}{\textbf{Understanding second partial derivatives}\\
...
\example{ex_partial8}{\textbf{Partial derivatives of functions of three variables}\\
...
$$f_{xyz}(x,y,z)  =\frac{\partial}{\partial z}\left(\frac{\partial}{\py}\left(\frac{\pf}{\px}\right)\right) \quad $$

\example{ex_partial9}{\textbf{Higher order partial derivatives}\\
\begin{enumerate}
