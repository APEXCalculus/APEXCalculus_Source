...
The point of substitution is to make the integration step easy. Indeed, the step $\int \Fp(u)\ du = F(u) + C$ looks easy, as the antiderivative of the derivative of $F$ is just $F$, plus a constant. The ``work'' involved is making the proper substitution. There is not a step--by--step process that one can memorize; rather, experience will be one's guide as they proceed. To gain experience, we now embark on many examples.\\

\example{ex_sub1}{\textbf{Integrating by substitution}\\
...
Let $u = x^2+5$, hence $du = 2xdx$. The integrand has an $xdx$ term, but not a $2xdx$ term. (Recall that multiplication is commutative, so the $x$ does not physically have to be next to $dx$ for there to be an $xdx$ term.) We can divide both sides of the $du$ expression by 2:
	$$du = 2xdx \quad \Rightarrow \quad \frac12du = xdx.$$ We can now substitute.
...
\example{ex_sub2}{\textbf{Integrating by substitution}\\
...
\example{ex_sub3}{\textbf{Integrating by substituting a linear function}\\Evaluate $\ds \int \frac{7}{-3x+1}\ dx$.}
...
\example{ex_sub10}{\textbf{Integrating by substitution}\\
...
\example{ex_sub4}{\textbf{Integrating by substitution}\\
...
\example{ex_sub5}{\textbf{Integrating by substitution}\\
...
\begin{align*}
	\int \frac1{x\ln x}\ dx 	&=	\int \frac{1}{\underbrace{\ln x}_u}\underbrace{\frac1x\ dx}_{du} \\
												&= \int \frac1u\ du \\
												&= \ln |u| + C \\
												&= \ln | \ln x| + C.
\end{align*}
...
A later section will delve deeper into integrals of a variety of trigonometric functions; here we use substitution to establish a foundation that we will build upon. 
...
\example{ex_sub6}{\textbf{Integration by substitution: antiderivatives of $\tan x$}\\
...
\example{ex_sub7}{\textbf{Integrating by substitution: antiderivatives of $\sec x$}\\
...
\example{ex_sub8}{\textbf{Integration by substitution: powers of $\cos x$ and $\sin x$}\\
...
\example{ex_sub9}{\textbf{Integration by substitution: simplifying first}\\
...
\example{ex_sub11}{\textbf{Integration by alternate methods}\\
...
\example{ex_subst14}{\textbf{Integrating by substitution: inverse trigonometric functions}\\
...
\example{ex_subst15}{\textbf{Integrating by substitution: inverse trigonometric functions}\\Evaluate the given indefinite integrals.
...
\example{ex_subst16}{\textbf{Integrating by substitution: completing the square}\\Evaluate $\ds \frac{1}{x^2-4x+13}\ dx$.}
...
\example{ex_subst17}{\textbf{Integrals require multiple methods}\\
...
\begin{enumerate}
\item		Start with definite integral $\ds \int_a^b f(x)\ dx$ that requires substitution.
...
\example{ex_sub12}{\textbf{Definite integrals and substitution: changing the bounds}\\
...
\example{ex_subst13}{\textbf{Definite integrals and substitution: changing the bounds}\\
