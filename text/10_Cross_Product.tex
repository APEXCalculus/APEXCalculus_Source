...
Given two, non--parallel vectors $\vec u$ and $\vec v$, it is very useful to find a vector $\vec w$ that is perpendicular to both $\vec u$ and $\vec v$. There is a operation, called the \textbf{cross product}, that creates such a vector. This section defines the cross product, then explores its properties and applications.
...
\example{ex_crossp1}{\textbf{Computing a cross product}\\
...
As each of these dot products is zero, $\crossp uv$ is indeed orthogonal to both $\vec u$ and $\vec v$.
}\\
...
\example{ex_crossp2}{\textbf{Computing a cross product}\\
...
\begin{enumerate}
	\item \parbox{200pt}{$\crossp uv = -(\crossp vu)$} Anticommutative Property
	\item	\begin{enumerate}
		\item \parbox{177pt}{$(\vec u+\vec v)\times \vecw = \crossp uw+\crossp vw$} Distributive Properties
		\item	$\vec u \times (\vec v+\vec w) = \crossp uv+\crossp uw$
	\end{enumerate}
	\item		$c(\crossp uv) = (c\vecu) \times \vec v = \vecu \times (c\vecv)$
	\item		\begin{enumerate}
		\item \parbox{177pt}{$(\crossp uv)\cdot \vecu = 0$} Orthogonality Properties
		\item	$(\crossp uv)\cdot \vecv = 0$
	\end{enumerate}
	\item		$\crossp uu = \vec 0$
	\item		$\crossp u0 = \vec 0$
	\item		\parbox{200pt}{$\vecu \cdot (\vecv\times\vecw) = (\crossp uv)\cdot \vecw$} Triple Scalar Product
\end{enumerate}
}
%\end{minipage}
\restoreboxwidth
...
\example{ex_crossp3}{\textbf{The cross product and angles}\\
...
\end{align*}
Numerically, they seem equal. The Exercise section offers an opportunity to prove the validity of Theorem \ref{thm:cross_product} in general.
}\\
...
\example{ex_crossp4}{\textbf{Finding the area of a parallelogram}\\
\begin{enumerate}
...
\example{ex_crossp5}{\textbf{Area of a triangle}\\
...
\example{ex_crossp6}{\textbf{Finding the volume of parallelepiped}\\
...
So the volume of the parallelepiped is 2 units.
...
\example{ex_crossp7}{\textbf{Computing torque}\\
...
While the cross product has a variety of applications (as noted in this chapter), its fundamental use is finding a vector perpendicular to two others. Knowing a vector orthogonal to two others is of incredible importance, as it allows us to find the equations of lines and planes in a variety of contexts. The importance of the cross product, in some sense, relies on the importance of lines and planes, which see widespread use throughout engineering, physics and mathematics. 
